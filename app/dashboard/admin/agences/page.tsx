    ...
    const HandlerSubmit = async (e: any) => {
        ...
        if (value != null) {
            try {
                ...
                if (res.ok) {
                    setIsOpen(false);
                    configPopup("Agence enregistré!", "green", "Reservation");
                    setValue(null);
        ...
        if (isOpen) {
            return (
                <div className="fixed bg-black/60 z-10 top-0 left-0 flex justify-center items-center w-full h-full">
                    <form onSubmit={HandlerSubmit} className="w-96 bg-white rounded-md overflow-hidden flex flex-col justify-between shadow-2xl">
                        <h2 className="p-4 bg-blue-500 font-bold items-center text-left flex justify-between  text-white uppercase">
                            Ajouter une agence
                            <button onClick={handleButtonClick} className="bg-white rounded-full hover:opacity-100 opacity-70 p-2"><Image src={closeSVG} width={15} height={15} alt="Image" /></button>
                        </h2>
                        <div className="p-4">
                            <div className="mt-4">
                            ...
                            <button type="submit" className="text-white w-full text-sm mt-4 hover:bg-blue-700 rounded-sm bg-blue-500  p-2">
                                Enregistrer
                            </button>
                        </div>
                    </form>
    ...
    const HandlerSubmitEdit = async (e: any) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                configPopup("Agence edité!", "green", "Reservation");
                setItemEdit(null);
            } else {
                configPopup("Erreur", "red", "Reservation");
            }
        ...
        return (
            <div className="fixed bg-black/60 z-10 top-0 left-0 flex justify-center items-center w-full h-full">
                <form onSubmit={HandlerSubmitEdit} className="w-96 bg-white rounded-md overflow-hidden flex flex-col justify-between shadow-xl">
                    <h2 className="   p-4 bg-green-500 font-bold text-left text-white uppercase">
                        Editer
                    </h2>
                    <div className="p-4">
                        <div className="mt-4">
                        ...
                        <button type="submit" className="text-white text-sm mt-4 hover:bg-green-700 rounded-sm bg-green-500  p-2">
                            Modifier
                        </button>
                        <button onClick={() => setIsOpenFormEdit(false)} className="hover:bg-stone-800 bg-stone-700 text-white text-sm p-2">Fermer</button>
                    </div>
                            ...
                            agences.map((item: any, index: number) => (
                                <tr key={index}>
                                    ...
                                    <td className="px-3 py-2 border flex gap-2">
                                        <button onClick={() => deleteAgence(item.id)} type="button" className="text-white text-sm hover:bg-red-700 rounded-sm bg-red-500  p-2">
                                            Supprimer
                                        </button>
                                        <button type="button" onClick={() => { setItemEdit(item); setIsOpenFormEdit(true) }} className="text-white text-sm hover:bg-yellow-700 rounded-sm bg-yellow-500  p-2">
                                            Modifier
                                        </button>
                                    </td>
                                </tr>
